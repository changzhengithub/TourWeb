	...
	methods: {
		...
		submitInformation: function () {
			...
			if (!_this.firstName) {
				...
				_this.tipMsg = '名を入力してください'
				return
			}
			if (!_this.lastName) {
				_this.isDisabled = false
				_this.tipColor = "#da4f49"
				_this.tipMsg = '姓を入力してください'
				return
			}
			if (!_this.email) {
				_this.isDisabled = false
				_this.tipColor = "#da4f49"
				_this.tipMsg = 'メールアドレスを入力してください'
				return
			}
			if (!_this.isEmail(_this.email)) {
				_this.tipMsg = 'メールアドレスは正しくありません'
				_this.tipColor = "#da4f49"
				_this.isDisabled = false
				return
			}
